...
def main_loop():
	...
	#tmp = frame_process[:, :, ::-1]
        #rgb_small_frame = np.ascontiguousarray(tmp)
        ...
        if allowedFaces > 0:
          if isShooting:
            isShooting = False;
            print("not shooting")
            data_str = f"stop from facial jetson"
            data = data_str.encode("utf-8")
            future = publisher.publish(topic_path, data)
            print(future.result())
        elif nonallowedFaces > 0:
          if not isShooting:
            isShooting = True;
            print("shooting")
            data_str = f"go from facial jetson"
            data = data_str.encode("utf-8")
            future = publisher.publish(topic_path, data)
            print(future.result())
